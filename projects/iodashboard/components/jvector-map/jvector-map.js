 ...
 * @see          http://jvectormap.com/
 * @copyright    2011-2013, Kirill Lebedev
 * @license      {@link http://jvectormap.com/licenses-and-pricing}
...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function ( searchElement, fromIndex ) {
	...
	degRad:      180 / Math.PI, radDeg: Math.PI / 180, radius: 6381372, sgn: function ( n ) {
		...
		var fi0 = 0, lambda0 = c * this.radDeg, fi1 = 29.5 * this.radDeg, fi2 = 45.5 * this.radDeg, fi = lat * this.radDeg,
		...
		var x = xCoord / this.radius, y = yCoord / this.radius, fi0 = 0, lambda0 = c * this.radDeg,
			...
			ro0 = Math.sqrt( C - 2 * n * Math.sin( fi0 ) ) / n, ro = Math.sqrt( x * x + (ro0 - y) * (ro0 - y) ),
			theta = Math.atan( x / (ro0 - y) );
		return { lat: Math.asin( (C - ro * ro * n * n) / (2 * n) ) * this.degRad, lng: (lambda0 + theta / n) * this.degRad }
	}, lcc:      function ( lat, lng, c ) {
		var fi0 = 0, lambda0 = c * this.radDeg, lambda = lng * this.radDeg, fi1 = 33 * this.radDeg, fi2 = 45 * this.radDeg,
			fi = lat * this.radDeg,
		...
		var x = xCoord / this.radius, y = yCoord / this.radius, fi0 = 0, lambda0 = c * this.radDeg, fi1 = 33 * this.radDeg,
			fi2 = 45 * this.radDeg,
			n = Math.log( Math.cos( fi1 ) * (1 / Math.cos( fi2 )) ) / Math.log( Math.tan( Math.PI / 4 + fi2 / 2 ) * (1 / Math.tan( Math.PI / 4 + fi1 / 2 )) ),
			F = Math.cos( fi1 ) * Math.pow( Math.tan( Math.PI / 4 + fi1 / 2 ), n ) / n,
			ro0 = F * Math.pow( 1 / Math.tan( Math.PI / 4 + fi0 / 2 ), n ),
	...
	bindElementEvents:          function () {
		...
		this.container.mousemove( function ( e ) {
			...
			var baseVal = jvm.$( this ).attr( "class" ).baseVal || jvm.$( this ).attr( "class" ),
				...
				tipShowEvent = jvm.$.Event( type + "TipShow.jvectormap" ), overEvent = jvm.$.Event( type + "Over.jvectormap" );
	...
	regionStyle:       {
		initial:  {
			...
			"stroke-width":   0,
			"stroke-opacity": 1
		}, hover: { "fill-opacity": .8, cursor: "pointer" }, selected: { fill: "yellow" }, selectedHover: {}
	},
	regionLabelStyle:  {
		initial:  {
			"font-family": "Verdana",
